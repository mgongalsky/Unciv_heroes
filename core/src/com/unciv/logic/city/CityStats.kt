...
class CityStats(val cityInfo: CityInfo) {
    ...
    /**
     * Updates the city's tile statistics by calculating and aggregating yields from worked tiles and other factors.
     *
     * This function determines the total yields (e.g., food, production, gold, etc.) generated by the tiles
     * within the city's range. It includes:
     * - Tiles that are explicitly worked by the city's population.
     * - The city's center tile.
     * - Tiles that provide yields without requiring population (e.g., certain improvements or terrain features).
     *
     * Additionally, the function calculates the production cost of tile improvements being constructed by hero units
     * on city-owned tiles. The production cost is subtracted from the total production yield.
     *
     * Finally, the aggregated statistics are stored in `statsFromTiles` for further use.
     */
    fun updateTileStats() {
        val stats = Stats()
        val localUniqueCache = LocalUniqueCache()
        ...
        // Calculate the cost of the improvement being done by heroes on the city tiles
        var improvementCost = 0f
        cityInfo.getTiles().filter {
            //it.militaryUnit?.action == UnitActionType.ConstructImprovement.value
